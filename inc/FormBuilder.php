...
class ThatFormBuilder {
	...
	function build_form( $echo = true ) {
		...
		foreach ( $this->inputs as $key => $val ) :
			...
			if ( isset( $_REQUEST[$val['name']] ) && ! empty( $_REQUEST[$val['name']] ) && $val['use_request'] ) {
			...
			switch ( $val['type'] ) :
				...
				case 'select':
					$element = 'select';
					$end     = '>' . $this->_output_options_select( $val['options'], $val['value'] ) . '
					</select>';
					break;
	...
	private function _output_options_select( $arr, $value ) {
		$output = '';
